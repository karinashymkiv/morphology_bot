...
async fn main() {
    ...
    println!("Loading the declension dictionary");
    ...
    println!("Len: {}", declension_file.noun_words.len());
    println!("WORD AT 10 POS: {:?}", declension_file.noun_words[10]);
    println!("Declension dictionary loaded");
    ...
    let quiz = quiz::Quiz::new(
        (0..amount)
            .map(|_| {
                ...
                random_noun.generate_question_out_of_noun()
            })
            .collect(),
    );
